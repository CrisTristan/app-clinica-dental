...
const duros = [
  ...
  { id: "raiz",    label: "Raíz"    },
  { id: "dentina", label: "Dentina" },
  { id: "huesos",  label: "Huesos"  },
] as const

const blandos = [
  { id: "encia",     label: "Encía"                   },
  { id: "insercion", label: "Inserción"                },
  { id: "epitelial", label: "Epitelial (Migración)"    },
  { id: "pulpa",     label: "Pulpa (Alteraciones)"     },
  { id: "velo",      label: "Velo de Paladar"          },
  { id: "carrillos", label: "Carrillos"                },
] as const

const oclusion = [
  { id: "sobre mordida", label: "Sobremordida Vertical" },
  { id: "desgaste",      label: "Desgaste"              },
  { id: "onoclusión",    label: "Onoclusión"            },
  { id: "intercurpideo", label: "Intercuspídeo"         },
  { id: "mareos",        label: "Mareos"                },
  { id: "desmayos",      label: "Desmayos"              },
  { id: "vertigos",      label: "Vértigos"              },
] as const
...
function ChipField<K extends keyof z.infer<typeof FormSchema>>({
  items, field,
}: {
...
export default function ExamenTejidos({ id }: { id: string | null }) {
  const form = useForm<z.infer<typeof FormSchema>>({
    ...
    defaultValues: { duros: [], blandos: [], oclusion: [] },
  })
  ...
  useEffect(() => {
    ...
    fetch(`/DentalData/api?id=${id}`)
      .then(r => r.json())
      .then(data => { if (data?.examenTejidos) form.reset(data.examenTejidos) })
      .catch(console.error)
  }, [])
  ...
  function onSubmit(data: z.infer<typeof FormSchema>) {
    fetch("/DentalData/api", {
      ...
      body: JSON.stringify({ examenTejidos: data, id }),
    })
      .then(r => { if (!r.ok) throw new Error(); return r.json() })
      .then(() => toast({ title: "Examen de tejidos guardado" }))
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <GroupLabel>Tejidos duros</GroupLabel>
              <ChipField items={duros} field={field} />
              <FormMessage />
            </FormItem>
          )}
        />
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <GroupLabel>Tejidos blandos</GroupLabel>
              <ChipField items={blandos} field={field} />
              <FormMessage />
            </FormItem>
          )}
        />
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <GroupLabel>Oclusión</GroupLabel>
              <ChipField items={oclusion} field={field} />
              <FormMessage />
            </FormItem>
          )}
        />
