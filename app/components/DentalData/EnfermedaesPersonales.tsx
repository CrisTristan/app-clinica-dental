...
const items = [
  { id: "aparato cardiovascular",  label: "Cardiovascular"         },
  { id: "sisteme nervioso",        label: "Sistema Nervioso"       },
  { id: "renal",                   label: "Renal"                  },
  { id: "aparato digestivo",       label: "Ap. Digestivo"          },
  { id: "aparato respiratorio",    label: "Ap. Respiratorio"       },
  { id: "diabetes",                label: "Diabetes"               },
  { id: "propencion hemorragica",  label: "Prop. Hemorrágica"      },
  { id: "artritis",                label: "Artritis"               },
  { id: "pruebas de laboratorio",  label: "Labs. de Laboratorio"   },
  { id: "estado general",          label: "Estado General"         },
  { id: "estudio radiologico",     label: "Estudio Radiológico"    },
] as const
...
export default function EnfermedadesPersonales({ id }: { id: string | null }) {
  ...
  useEffect(() => {
    ...
    fetch(`/DentalData/api?id=${id}`)
      .then(r => r.json())
      .then(data => { if (data?.enfermedadesPersonales) form.reset(data.enfermedadesPersonales) })
      .catch(console.error)
  }, [])
  ...
  function onSubmit(data: z.infer<typeof FormSchema>) {
    fetch("/DentalData/api", {
      ...
      body: JSON.stringify({ enfermedadesPersonales: data, id }),
    })
      .then(r => { if (!r.ok) throw new Error(); return r.json() })
      .then(() => toast({ title: "Enfermedades guardadas" }))
