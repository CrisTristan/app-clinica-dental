...
import {
  Activity, Stethoscope, User, Calendar, ListChecks, ChevronRight,
...
export default function TratamientosActivos({ id }: { id: string | null }) {
  ...
  //   "sheet" (vista 3) → se muestra la hoja de consentimiento (tratamiento general).
  //   "none"          → fuera del flujo (vista de detalle).
  const [consentStep, setConsentStep] = useState<"none" | "sheet">("none")

  // Documento de consentimiento (vista 3): datos, texto, firma y modo edición.
  ...
  const view: "list" | "detail" | "consentSheet" =
    selectedId == null ? "list"
      : consentStep === "sheet" ? "consentSheet"
      : "detail"

  // Abre la hoja de consentimiento del tratamiento general (todos los procedimientos).
  const openConsentSheet = () => {
    if (!detail) return
    setConsentData(crearHojaConsentimientoData({
      odontologo: detail.dentista ?? "",
      paciente: detail.paciente ?? "",
      procedimientos: detail.procedures.map(p => ({ id: crypto.randomUUID(), nombre: p.nombre, detalle: "" })),
    }))
  ...
  const isConsent = view === "consentSheet"

  // El botón "atrás" retrocede un paso según la vista activa.
  const onBack =
    view === "consentSheet" ? () => setConsentStep("none")
      : backToList
  ...
  // Botones de acción del encabezado según la vista.
  const headerActions =
    view === "detail" && detail?.has_consent ? (
      // Acceso a la hoja de consentimiento cuando el tratamiento ya tiene una.
      <button
        type="button"
        onClick={openConsentSheet}
        title="Hoja de consentimiento"
        className="inline-flex items-center gap-1.5 rounded-lg border border-sky-200 dark:border-sky-800
      ...
      // Barra de acciones de la hoja: editar / imprimir / guardar.
      <>
        <button
          type="button"
  ...
  return (
    <VentanaPopup
      ...
      icon={isConsent ? FileCheck : Activity}
      title={title}
      subtitle={subtitle}
      onBack={view === "list" ? undefined : onBack}
      actions={headerActions}
      // En la vista 4, al editar, se ensancha en escritorio para ver el
      // formulario y la vista previa lado a lado con comodidad.
      contentClassName={view === "consentSheet" && editingConsent ? "sm:max-w-2xl lg:max-w-5xl" : "sm:max-w-2xl"}
    >
        ...
        {/* ── Vista 3: vista previa (y edición) de la hoja de consentimiento ── */}
        {view === "consentSheet" && consentData && (
